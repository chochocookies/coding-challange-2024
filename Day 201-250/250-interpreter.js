...
function interpreter(code, tape) {
     // Convert tape string to array of bits
     let tapeArray = tape.split('').map(Number);
     let pointer = 0;
     const codeLength = code.length;
     const bracketMap = createBracketMap(code);
     ...
     for (let i = 0; i < codeLength; i++) {
         ...
         switch (command) {
             ...
             case '<':
                 ...
                 break;
             case '*':
                 tapeArray[pointer] = tapeArray[pointer] === 0 ? 1 : 0; // Flip the bit
                 break;
             case '[':
                 if (tapeArray[pointer] === 0) {
                     i = bracketMap[i]; // Jump to the matching ]
                 }
                 break;
             case ']':
                 if (tapeArray[pointer] !== 0) {
                     i = bracketMap[i]; // Jump back to the matching [
                 }
                 break;
             default:
 ...
 // Helper function to create a mapping of brackets
 function createBracketMap(code) {
     const stack = [];
     const bracketMap = {};
     ...
     for (let i = 0; i < code.length; i++) {
         if (code[i] === '[') {
             ...
             bracketMap[start] = i; // Map the opening bracket to the closing bracket
             bracketMap[i] = start; // Map the closing bracket to the opening bracket
         }
     }
